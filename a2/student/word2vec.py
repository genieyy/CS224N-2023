...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    Ut,o,vc=outsideVectors,outsideWordIdx,centerWordVec
    yhat=softmax(np.matmul(Ut,vc))
    loss=-np.log(yhat[o])
    y_tru=np.eye(Ut.shape[0])[o]
    gradCenterVec=Ut.T@(yhat-y_tru)
    gradOutsideVecs=np.outer(yhat-y_tru,vc)
